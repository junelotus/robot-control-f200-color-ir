...
{//添加摄像头得到的距离信息进行控制
  ...
  detect de;
  calXYZ cal;
  while (1&&cv::waitKey(30) !=27) {//顯示圖像並且將圓形匹配出來
    dev->wait_for_frames();
    ...
    if(currentTimestamp[0] != lastTimestamp[0]){
       ...
       if(coo.size()>0&&coo[0].size()>0&&cm730.Connect() == true)
        {
           
           int len=coo[0].size()-1;
           cout<<"len="<<len<<endl;
           pair<float,float> d_a=cal.calAngle(len,coo[0],coo[1],coo[2] );
           cout<<"dis:"<<d_a.first<<"    angle:"<<d_a.second<<endl;
           Scan(&cm730);
	   Parainit();
	   stand();
	   ...
	   if(d_a.second>0.0)
          {
           cout<<"angle"<<d_a.second<<endl;
           mWalk.X0 = 0;				
           mWalk.StepCountTarget = 3;

	   mWalk.RobotYaw_New = (double)(d_a.second/PI*180.0)/2;
           mWalk.RobotYaw_Temp = (double)(d_a.second/PI*180.0)/2;
           mWalk.PatternInit(); 
           mWalk.RobotState=mWalk.Walk_start;	
    ...
    if(currentTimestamp[1] != lastTimestamp[1]){
        ...
        depthData = (void *)dev->get_frame_data(rs::stream::infrared2);
        depthImg = cv::Mat(480, 640, CV_8U,depthData).clone();
    }
