...
int main(int argc, char **argv)
{
	...
	//image_transport::ImageTransport it(nh);
       ...
       cv::Mat depthImg, filterImg,rgbImg,irImg,depth_at_colorImg;//show   
    ...
    detect de;
    void *rgbData, *depthData,*irData,*depth_at_colorData;
  //  sensor_msgs::ImagePtr rgbMsg, depthMsg;
   const float scale = dev->get_depth_scale();

    rs::intrinsics depth_intrin = dev->get_stream_intrinsics(rs::stream::depth);
    rs::extrinsics depth_to_color = dev->get_extrinsics(rs::stream::color,rs::stream::depth);
    rs::intrinsics color_intrin = dev->get_stream_intrinsics(rs::stream::color);
    rs::intrinsics ir_intrin = dev->get_stream_intrinsics(rs::stream::infrared );
    rs::intrinsics depth_aligned_to_color_intrin =dev->get_stream_intrinsics(rs::stream::depth_aligned_to_color );
	dev->wait_for_frames();
		...
		depth_at_colorData=(void *)dev->get_frame_data(rs::stream::depth_aligned_to_color);
		depth_at_colorImg=cv::Mat(480, 640, CV_16U,depth_at_colorData).clone();
   ...
   /*cam  control*/
  	...
  	if(cm730.Connect() == true)
	{
  	...
  	int count=1;
 	while (nh.ok()&&cv::waitKey(30) !=27)//&&cv::waitKey(30) !=27) 
  	  {//顯示圖像並且將圓形匹配出來
