...
*/
// static vector<pair<cv::Point,cv::Point>> matches;

 float ir_to_color_baselineX=25.7;//mm
 float ir_to_color_baselineY=-0.733261 ;//mm          
 float ir_to_color_baselineZ=3.95646;//mm  about z's  position or un[osition
 float  H=250.0f+4.175f;//400.00f;//攝像頭距離地點的高度 
 //const float  T=25.7;//mm   70.068;//78.18f;//兩個攝像頭之間的距離寬度
 float  f=619.695;//mm   589.201294;//456.35427;//595.0f;//攝像頭的焦距  img_color 
 float  f_ir=475.045;//						  img_ir
 float  imgWeidth=640.0;
 float  imgHeigth=480.0;
 float  xl=0.0;
// static  float  T;//=0.0;//ir_to_color_baselineX+(ir_to_color_baselineZ*(320.0-xl)/f);//
 vector<float> z;
 vector<float> x;
 vector<float> y;
...
/*calXYZ(vector<pair<cv::Point,cv::Point>  > matches)
{

this->matches= matches;
this->xl=(float)matches[0].first.x;
//this->xl=(float)matches[0].first.x;
this->T=ir_to_color_baselineX+(ir_to_color_baselineZ*(imgWeidth/2.0-this->xl)/f);//
this->matches[0].first.x=matches[0].first.x +(f*(imgWeidth/2.0-this->xl)/( f-ir_to_color_baselineZ-f_ir) ) ;//chong zhi 
this->xl=this->matches[0].first.x;

}*/


vector<float> calZ( vector<pair<cv::Point,cv::Point> > matches,const int i,float T)
{//cal.matches
...
vector<float> calZ( vector<pair<cv::Point,cv::Point> >  matchesTmp,float &T,Mat & img,const int )
	{//matches & changed the value,so the value of matches is changed in the next function coming  
		cout<<"T:"<< T<<endl;
	 
		vector<pair<cv::Point,cv::Point> > matches= matchesTmp;
		vector<float> z;
		float sum=0.0;
		int size=matchesTmp.size();
		float depth=0; 
		for(int i=0;i<size;i++)
		 { 
		T=ir_to_color_baselineX-(ir_to_color_baselineZ*(imgWeidth/2.0-matches[i].first.x)/f); // changed
		/* float ir_to_color_baselineX=25.7;//mm
		 float ir_to_color_baselineY=-0.733261 ;//mm          
		 float ir_to_color_baselineZ=3.95646;//mm
		 float  H=250.0f+4.175f;//400.00f;//攝像頭距離地點的高度 
		 //const float  T=25.7;//mm   70.068;//78.18f;//兩個攝像頭之間的距離寬度
		 float  f=619.695;//mm   589.201294;//456.35427;//595.0f;//攝像頭的焦距  img_color 
		 float  f_ir=475.045;//	*/
		float wucha=(ir_to_color_baselineZ*(imgWeidth/2.0-matches[i].first.x)/f);//correct the T wucha
		 cout<<"Z["<<i<<"] T:"<<T<<endl; // changed<<end;
		 
		 matches[i].first.x=imgWeidth/2.0-( (imgWeidth/2.0-matches[i].first.x)*(f_ir-ir_to_color_baselineZ)/f ) ;//chong zhi


		float yuce=matchesTmp[i].first.x+(imgWeidth/2.0-matchesTmp[i].first.x)*(f-f_ir+ir_to_color_baselineZ)/f;

/*matches[i].first.x=(float)matches[i].first.x +( (imgWeidth/2.0-(float)matches[i].first.x)*(  f-ir_to_color_baselineZ-f_ir)/f ) ;//chong zhi 
		float yuce=imgWeidth/2.0-( (imgWeidth/2.0-matchesTmp[i].first.x)*( f_ir+ir_to_color_baselineZ)/f ) ;//chong zhi  f-ir_to_color_baselineZ-f_ir*/

cout<<"1:"<<matches[i].first.x<<"   2:"<<yuce<<endl;
			float xl=matches[i].first.x;		       
			float xr=matches[i].second.x;
		       //cout<<"xl="<<xl<<"  ";
		      
		       // float xr=xR-(640.0/2-xR)*(f-f_ir)/f_ir;//
		       //cout<<"xr="<<xr<<"  ";
		       sum+=abs(f_ir*T/(xl-xr-wucha));
		       // cout<<"class:z="<<f*T/(xl-xr)<<endl;
		       z.push_back(abs(f_ir*T/(xl-xr-wucha)));//z.push_back(abs(f_ir*T/(T-ir_to_color_baselineX+xl-xr)));
		 }
  
	       z.push_back(sum);
	       depth=z[z.size()-1]/(float)(z.size()-1);
		  cout<< "Z matches[0]:("<< matches[0].first.x<<" ,"<<matches[0].first.y<<"),("<<matches[0].second.x<<","<<matches[0].second.y<<")"<<endl; 	
	 cout<< "Z matches[1]:("<< matches[1].first.x<<" ,"<<matches[1].first.y<<"),("<<matches[1].second.x<<","<<matches[1].second.y<<")"<<endl;
	// cv::circle(img,matches[0].first,3, cv::Scalar(0,0,255), -1, 8, 0);
         cv::circle(img,matches[1].first,3, cv::Scalar(255,0,0), -1, 8, 0);
	      // cout<<"depth"<<depth<<endl;
	       //this->z=z;
		//matches=  tem;
	       return z;
	...
	vector<float> calZ( vector<pair<cv::Point,cv::Point> >  matchesTmp,float &T,Mat & img)
	{//matches & changed the value,so the value of matches is changed in the next function coming  
		cout<<"T:"<< T<<endl;
		...
		/* float ir_to_color_baselineX=25.7;//mm
		 ...
		 float  f=619.695;//mm   589.201294;//456.35427;//595.0f;//攝像頭的焦距  img_color 
		 float  f_ir=475.045;//	*/
		float wucha=(ir_to_color_baselineZ*(matches[i].first.x-imgWeidth/2.0)/f);//correct the T wucha
		 cout<<"Z["<<i<<"] T:"<<T<<endl; // changed<<end;
	...
	cout<<"1:"<<matches[i].first.x<<"   2:"<<yuce<<endl;
			...
			float xr=matches[i].second.x;
		       ...
		       //cout<<"xr="<<xr<<"  ";
		       	sum+=abs(f_ir*T/(xl-xr-wucha));
			cout<<"Z:"<<f_ir*T/(xl-xr-wucha)<<endl;
		       // cout<<"class:z="<<f*T/(xl-xr)<<endl;
		       z.push_back(abs(f_ir*T/(xl-xr-wucha)));//z.push_back(abs(f_ir*T/(T-ir_to_color_baselineX+xl-xr)));
		 }
	...
	vector<float> calX(vector<float> z, vector<pair<cv::Point,cv::Point> > matchesTmp,float &T)
	{  vector<float> x;  
	   int size=matchesTmp.size();
	   float sum=0;
	   vector<pair<cv::Point,cv::Point> > matches=matchesTmp;
	   for(int i=0;i<size;i++)
		...
		T=ir_to_color_baselineX+(ir_to_color_baselineZ*(imgWeidth/2.0-matches[i].first.x)/f); // changed
		// cout<<"X["<<i<<"] T:"<<T<<endl; // changed<<end;
		 
		 matches[i].first.x=matches[i].first.x +( (imgWeidth/2.0-matches[i].first.x)*( f-ir_to_color_baselineZ-f_ir)/f ) ;//chong zhi

	float wucha=(imgWeidth/2.0-matches[i].first.x)*z[i]/(f_ir+ir_to_color_baselineZ);
      //  cout<<matches[i].first.x<<"   "<<matches[i].second.x<<endl;
	//T=ir_to_color_baselineX+(ir_to_color_baselineZ*(imgWeidth/2.0-matches[i].first.x)/f_ir); // changed
   
	  if((float)matches[i].first.x<imgWeidth/2.0)
		x.push_back(-abs((imgWeidth/2.0-(float)matches[i].first.x-wucha)*z[i]/f_ir)-(T/2.0));
		//x.push_back(-abs((imgWeidth/2.0-(float)matches[i].first.x)*z[i]/f_ir)-(T/2.0));
		...
		x.push_back(-abs((T/2-((float)matches[i].first.x-imgWeidth/2.0+wucha)*z[i]/f_ir)));
		//x.push_back(-abs((T/2-((float)matches[i].first.x-imgWeidth/2.0)*z[i]/f_ir)));
		...
		x.push_back(abs(T/2.0-(imgWeidth/2.0-(float)matches[i].second.x)*z[i]/f_ir));
		...
		//x.push_back(abs(((float)matches[i].first.x-imgWeidth/2.0)*z[i]/f-(T/2)));
                x.push_back(abs(z[i]/f_ir*((float)matches[i].second.x-imgWeidth/2.0)+(T/2)));
  ...
  //     cout<<"x:"<<sum/(float)(x.size()-1)<<endl;
    /* cout<< "X matches[0]:("<< matches[0].first.x<<" ,"<<matches[0].first.y<<"),("<<matches[0].second.x<<","<<matches[0].second.y<<")"<<endl; 	
	 cout<< "X  matches[1]:("<< matches[1].first.x<<" ,"<<matches[1].first.y<<"),("<<matches[1].second.x<<","<<matches[1].second.y<<")"<<endl;
   */ return  x;//sum/(float)(x.size()-1);


}

	vector<float> calY(vector<float> z, vector<pair<cv::Point,cv::Point> > matchesTmp,float &T)
	{
//在保持攝像頭是水平方向的情況下
   vector<float> y;
   int size=matchesTmp.size();
   ...
   for(int i=0;i<size;i++)
    {
	T=ir_to_color_baselineX+(ir_to_color_baselineZ*(imgWeidth/2.0-matches[i].first.x)/f); // changed
	//cout<<"Y["<<i<<"] T:"<<T<<endl; // changed<<end;
	 matches[i].first.x=matches[i].first.x +( (imgWeidth/2.0-matches[i].first.x)*( f-ir_to_color_baselineZ-f_ir)/f ) ;//chong zhi
     if(matches[i].first.y<imgHeigth/2.0)//正的位置
        y.push_back(abs((imgHeigth/2.0-matches[i].first.y)*z[i]/f_ir));  
     else if(matches[i].first.y==imgHeigth/2.0)
        y.push_back(0.0);
     else
        y.push_back(-abs((matches[i].first.y-imgHeigth/2.0)*z[i]/f_ir));  
     sum+=y[y.size()-1];   
     //cout<<"class:y="<<y[y.size()-1]<<endl;
      }

     y.push_back(sum);
/* cout<< "Y matches[0]:("<< matches[0].first.x<<" ,"<<matches[0].first.y<<"),("<<matches[0].second.x<<","<<matches[0].second.y<<")"<<endl; 	
	 cout<< "Y  matches[1]:("<< matches[1].first.x<<" ,"<<matches[1].first.y<<"),("<<matches[1].second.x<<","<<matches[1].second.y<<")"<<endl;
    //this->x=x;
    //cout<<"y:"<<sum/(float)(y.size()-1)<<endl;
  */
  return y;// sum/(float)(y.size()-1);
...
vector<float> calX( vector<pair<cv::Point,cv::Point> > matches,float T)
{ 
   vector<float> x;  
   int size=matches.size();
   float sum=0;
    
	//T=ir_to_color_baselineX+(ir_to_color_baselineZ*(imgWeidth/2.0-this->xl)/f);//
	//matches[0].first.x=matches[0].first.x +(f*(imgWeidth/2.0-this->xl)/( f-ir_to_color_baselineZ-f_ir) ) ;//chong zhi
...
vector<float> calY( vector<pair<cv::Point,cv::Point> > matches,float T)
{
//在保持攝像頭是水平方向的情況下
   vector<float> y;
   int size=matches.size();
...
//float calXYZ::T=0.0;
//vector<pair<cv::Point,cv::Point>> calXYZ::matches=vector<pair<cv::Point,cv::Point>>(0);
