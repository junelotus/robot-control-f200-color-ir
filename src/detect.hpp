...
vector<pair<cv::Point,cv::Point> > matches;
...
/*calXYZ cal;
detect(){}
detect(vector<pair<cv::Point,cv::Point> > matches)
{
...
for(  i=beginCol;i<640;i++)
  ...
  for(int j=0;j<480;j++)
   ...
   //   cout<<"66666666666666666666"<<endl;
   	 const uint8_t* rowPtr = img.ptr<uint8_t>(j);
   	 uint8_t pixel=rowPtr[i];
   	 if((int)pixel>230)//isWhite( j, i, img))//(
   	  if(begin0<0)//(int)pixel>250&&
   	    {
		begin0=i;p[0]=(float)begin0;
		break;
  	     }
...
//centre is correct
	...
	centre.push_back(p);
	p[1]=(centreR[1][0]+centreR[1][1])/2;//第幾行表示縱座標
	p[0]=(centreR[0][1]);//+centreR[0][1])/2;//第幾列表示橫座標
	//cv::circle(img1,cv::Point(p[0],p[1]),3, cv::Scalar(0, 0, 0), -1, 8, 0); 
	centre.push_back(p);
...
if(centre.size()==2){
	...
	p[0]=(centreRL[0][1]);//+centreRL[0][1])/2;
	centre.push_back(p);
      ...
      if(centreRR.size()>=2)
      {
	p1[1]=(centreRR[1][0]+centreRR[1][1])/2;
	p1[0]=(centreRR[0][1]);//+centreRR[0][1])/2;
			
	while(centreRR.size()>=2&&abs(abs(p1[0]-p[0])-abs(centre[0][0]-centre[1][0]))>=10 )
	{
	centreRR=rgbCentreDetect(img2,centreRR[0][1]+1);
        if(centreRR.size()>=2)
	{
	p1[1]=(centreRR[1][0]+centreRR[1][1])/2;
	p1[0]=(centreRR[0][1]);//+centreRR[0][1])/2;
	...
	//cv::circle(img2,cv::Point(p[0],p[1]),3, cv::Scalar(255, 255, 255), -1, 8, 0); 
	
	if(centreRR.size()>=2)
	//cv::circle(img2,cv::Point(p1[0],p1[1]),3, cv::Scalar(255, 0, 0), -1, 8, 0); 
	 centre.push_back(p1);
...
vector<Vec3f> centre(Mat &img1,Mat &img2,const int)
{
	Vec3f p=Vec3f(-1.0,-1.0,-1.0);
	Vec3f p1=Vec3f(-1.0,-1.0,-1.0);
      vector<Vec3f> centre;
...
vector<vector<float> > resultXYZ(vector<Vec3f> centre,Mat &img1,Mat &img2,const struct  intrinsics  color_intrin,const struct  intrinsics  ir_intrin,const struct  extrinsics color_to_ir_extrin) 
{
...
if(centre.size()==4)
{
circle.push_back(centre[2]);
circle.push_back(centre[3]);
circleR.push_back(centre[0]);
circleR.push_back(centre[1]);
return resultXYZ( circle ,  circleR,img1,img2,color_intrin, ir_intrin, color_to_ir_extrin) ;
}
if(centre.size()==2)
{
...
/*else{ circle.push_back(Vec3f(0.0,0.0,0.0));
circle.push_back(Vec3f(50.0,50.0,0.0));
circleR.push_back(Vec3f(10.0,10.0,0.0));
circleR.push_back(Vec3f(80.0,80.0,0.0));
return resultXYZ( circle ,  circleR,img1,img2,color_intrin, ir_intrin, color_to_ir_extrin) ;}*/
...
/*返回值是一個圓心半徑的數組每個數組裏面有三個值分別表示(x,y,r)*/
  vector<Vec3f>  circleDetect(Mat img)
   ...
   cv::HoughCircles(img, circles,CV_HOUGH_GRADIENT,1.5, 10, 200, 100, 0, 0); 
   ...
   if(circles.size()!=0)
     ...
     cv::circle(img, cv::Point(circles[i][0],circles[i][1]), 3, Scalar(255, 255, 255), -1, 8, 0);   
...
vector<vector<float> > resultXYZ(vector<Vec3f> circle , vector<Vec3f> circleR,Mat &img1,Mat &img2,const struct  intrinsics  color_intrin,const struct  intrinsics  ir_intrin,const struct  extrinsics color_to_ir_extrin) 
{     
	//cout<<"11111111111111111"<<endl;	
       ...
       vector<pair<cv::Point,cv::Point> > matches;//存放的是相互匹配的點對，一共兩對，每隊中是一對左右圖像上相互匹配的圖像
       cv::Point left,right;
       int first,second;
       ...
       matches.push_back(std::pair<cv::Point,cv::Point>(left,right));
        cv::circle(img2,left,3, cv::Scalar(0, 0, 0), -1, 8, 0);
	cv::circle(img1,right,3, cv::Scalar(0), -1, 8, 0);
       ...
       //cout<<"right:"<<right<<endl;;
        matches.push_back(std::pair<cv::Point,cv::Point>(left,right));
	cv::circle(img2,left,3, cv::Scalar(255, 255, 255), -1, 8, 0);
	cv::circle(img1,right,3, cv::Scalar(130), -1, 8, 0);
	/* cout<< "matches[0]:("<< matches[0].first.x<<" ,"<<matches[0].first.y<<"),("<<matches[0].second.x<<","<<matches[0].second.y<<")"<<endl;
        cout<< "matches[1]:("<< matches[1].first.x<<" ,"<<matches[1].first.y<<"),("<<matches[1].second.x<<","<<matches[1].second.y<<")"<<endl;
      */
       //cout<<"matches.size():"<<matches.size()<<endl;
	 calXYZByDiff cbd;
	calXYZ cal;//(matches);
	float T=cal.ir_to_color_baselineX;       	
	//cout<<"T:"<< T<<endl;
	//vector<float> z=cal.calZ(matches,T,img2,1);//matches
	//vector<float> z=cbd.calZ(matches,T,img2,1);

	 vector<float> z=cbd.calZ( matches,color_intrin,ir_intrin,color_to_ir_extrin);
	// cout<<"changed T:"<< T<<endl;
...
/*cout<<"ir_to_color_baselineX:"<<cal.ir_to_color_baselineX<<endl;
cout<<"ir_to_color_baselineY:"<<cal.ir_to_color_baselineY<<endl;
cout<<"ir_to_color_baselinez:"<<cal.ir_to_color_baselineZ<<endl;
  */   // cal.T=0.0;
	...
	/*vector<float>  x=cal.calX( z,matches,T);//,matches
	vector<float> y=cal.calY( z,matches,T);//,matches   
       p.x=x[0];
       p.z=z[0];    
       p.y=y[0];
       cout<<"p[0"<<"]:"<<p<<endl;
...
if(len==1&&lenR==1)
       ...
       //cout<<"right:"<<right<<endl;;
       matches.push_back(std::pair<cv::Point,cv::Point>(left,right));
	cv::circle(img2,left,3, cv::Scalar(255, 0, 0), -1, 8, 0);
	cv::circle(img1,right,3, cv::Scalar(0), -1, 8, 0);
